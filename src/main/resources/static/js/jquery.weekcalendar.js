...
(function($) {
	...
	$.widget("ui.weekCalendar", {
		...
		options: {
			...
			buttonText: {
				today: "",
				lastWeek: "<img src='images/toLeft.png' />",
				nextWeek: "<img src='images/toRight.png' />"
			},
		...
		_renderCalendar: function() {
			...
			if(options.buttons) {
				...
				$calendarContainer.find(".wc-nav .wc-prev").on('click', function() {
...
//					return false;
				...
				$calendarContainer.find(".wc-nav .wc-next").on('click', function() {
					clearTimeout(CountDown);
...
//					return false;
		...
		_updateDayColumnHeader: function($weekDayColumns) {
			...
			self.element.find(".wc-header td.wc-day-column-header").each(function(i, val) {
				...
				$(this).html( "<div class='weeks' d='"+self._formatDate(currentDay,'Y-M-d')+"'>"+ dayName + "</div>" + self._formatDate(currentDay, options.dateFormat));
				datess.push(self._formatDate(currentDay, options.dates));
			...
			$('#date_front').html(datess[0]);
			$('#date_back').html(datess[6]);
			$('#tab>li').html(datess[$('.wc-today').index() - 1]);
			var urls = 'http://www.youwenedu.com:8084/';
			$('.wc-day-column-header').on('click', function() {
				clearTimeout(CountDown);
				$(this).addClass('colr').siblings().removeClass('colr');
				var index = $(this).index() - 1;
				var ymd = [];
				var hms = [];
				ymd = datess[index].split('-');
				ymd[1] = parseInt(ymd[1] - 1);
				ymd[2] = parseInt(ymd[2]);
				var newd = [];
				var $tder = $('#sTmain>tbody>tr');
				var reduce = [];
				$('#tab>li').html(datess[index]);
//				subtable();
				wtime();
				function wtime() {
					var chad = new Date();
					var $tder = $('#sTmain>tbody>tr');
					reduce = [];
					for(i = 0; i < $tder.length; i++) {
						hms[i] = [];
						hms[i] = $tder.eq(i).find('td:eq(1)').text().split(':');
						hms[i][0] = parseInt(hms[i][0]);
						hms[i][1] = parseInt(hms[i][1]);
						newd.push(new Date(ymd[0], ymd[1], ymd[2], hms[i][0], hms[i][1], 0));
					}
			...
			$('.wc-header .wc-today').trigger('click');
